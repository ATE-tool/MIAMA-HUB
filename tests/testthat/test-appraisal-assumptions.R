...
test_that("e-bike cycling proxies apply explicit factors once and preserve edits", {
  cfg <- miama_default_config()
  p <- prepare_assumption_profile(list(modes = "ebike"),
                                 source_data = assumption_fixture(), cfg = cfg)
  expect_equal(p$default_trip_distance_ebike$default_value, 4 * 1.3)
  expect_equal(p$default_trip_duration_ebike$default_value, 20 * 1.3 / 1.2)
  expect_equal(p$assump_trip_speed_ebike$default_value, 15.7 * 1.2)
  expect_equal(p$default_trip_distance_ebike$assumption$proxy, "Cycling proxy")
  p$default_trip_distance_ebike$is_filled <- TRUE
  p$default_trip_distance_ebike$input_value <- 9
  p <- prepare_assumption_profile(p, source_data = assumption_fixture(), cfg = cfg)
  expect_equal(p$default_trip_distance_ebike$default_value, 5.2)
  expect_equal(extract_input_values(p)$default_trip_distance_ebike, 9)
  expect_equal(cfg$assumptions$fixed$duration$ebike,
               cfg$assumptions$fixed$distance$ebike / cfg$assumptions$fixed$speed$ebike * 60)
})
