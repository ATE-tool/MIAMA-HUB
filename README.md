...
bounded below by every explicit mode-user count. It can exceed the source
population: HUB then creates separately identified copies of eligible donors.
...
3. Expand the eligible donor pool if necessary, then select uniquely identified
   REF records while meeting mode-user margins and retaining required trip owners.
...
1. **REF uses source evidence.** REF inputs select or copy eligible source
   people and trip patterns. A mode-user count cannot exceed REF total population.
   A missing native e-bike user pool can use explicitly labelled cycling proxies;
   their source health histories are also proxies, not observed e-bike evidence.
...
- **Stale REF total after category selection:** HUB reconstructs REF and CF
  ...
  the supplied table targets still exceed distinct eligible capacity, HUB reuses
  eligible donors instead of clipping solely to the source size. Excluded
  categories remain excluded; no eligible evidence still produces an error.
...
Expected health outcomes are summed over appraisal people with unit weight by
default. The factor and source are recorded in `results_report`. Source-derived
default counts describe the available record-sized example appraisal, not the
entire LAD. Full geographic population estimates in Tab 1 are context only;
larger appraisals require larger submitted counts, with donor reuse as needed.
Trip counts likewise remain row targets and row totals; source trip weights are
not applied as an additional expansion multiplier.
...
rows. Its geographic expansion factor is `163.552` (`20 / sample fraction`),
describing an 817,760-person source-population estimate. This is metadata only:
the default appraisal has 5,000 people, and health results use unit weight.
