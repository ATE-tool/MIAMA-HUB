    ...
    for (i in seq_len(nrow(catalog))) {
      ...
      if (quantity != "speed") {
        ...
        if (is.null(value) && mode == "ebiking") {
          value <- .assumption_observed(reference_data, "cycling", quantity)
          if (!is.null(value)) proxy <- "Cycling proxy"
        }
        if (!is.null(value)) origin <- "REF population"
        if (is.null(value)) {
          value <- .assumption_observed(source_data, mode, quantity)
          if (is.null(value) && mode == "ebiking") {
            value <- .assumption_observed(source_data, "cycling", quantity)
            if (!is.null(value)) proxy <- "Cycling proxy"
          }
          if (!is.null(value)) origin <- "Source population"
        }
        if (!is.null(value) && nzchar(proxy)) {
          spec <- utils::modifyList(miama_default_config()$counterfactual$modes$ebiking,
                                   cfg$counterfactual$modes$ebiking %||% list())
          factor <- switch(quantity, distance = spec$proxy_distance_factor,
                           duration = spec$proxy_duration_factor, 1)
          value <- value * factor
        }
        if (is.null(value)) {
